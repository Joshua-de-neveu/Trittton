...
export function Header({ onScrapeClick, scrapeRunning, term, onTermChange, termOptions, onLogout, userDisplayName, userPhotoURL, onToggleSidebar, theme, onToggleTheme, cloudStatus }: HeaderProps) {
  return (
    <header className="h-14 px-2 sm:px-4 flex items-center justify-between sticky top-0 z-50 border-b border-border"
      style={{
    ...
    >
      ...
      <div className="flex items-center gap-1.5 sm:gap-3 min-w-0">
        {onToggleSidebar && (
          <button onClick={onToggleSidebar} aria-label="Toggle navigation" className="p-1.5 rounded-lg text-muted hover:text-text hover:bg-card cursor-pointer shrink-0">
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="font-semibold text-[15px] text-text tracking-tight truncate">Trittton</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 sm:gap-2 shrink-0">
        ...
        <select
          ...
          className="bg-card border border-border rounded-lg text-[12px] sm:text-[13px] text-text
            px-2 sm:px-3 py-1.5 outline-none cursor-pointer focus:border-accent hover:border-border2 max-w-[120px] sm:max-w-none truncate"
        >
          {termOptions.map((t) => (
        ...
        <button
          ...
          aria-label={scrapeRunning ? 'Scraping…' : 'Scrape latest course data'}
          className="flex items-center gap-1.5 px-2 sm:px-3.5 py-1.5 rounded-lg text-[12px] sm:text-[13px] font-semibold
