// Auto-generated from catalog.ucsd.edu — DO NOT EDIT MANUALLY
// 7289 courses, 2218 with prerequisites

export type PrereqGraph = Record<string, string[]>

// courseCode -> [prerequisite course codes]
...
export function getUnlocks(courseId: string): string[] {
  ...
  for (const [code, prereqs] of Object.entries(PREREQ_GRAPH)) {
    if (prereqs.includes(courseId)) result.push(code)
  }
  return result
}
...
export function getDepth(courseId: string, visited = new Set<string>()): number {
  ...
  const prereqs = PREREQ_GRAPH[courseId]
  if (!prereqs || prereqs.length === 0) return 0
  return 1 + Math.max(...prereqs.map(p => getDepth(p, visited)))
}
